...
def lambda_handler(event, context):
    ...
    agent_kb_response = bedrock_client.get_agent_knowledge_base(
        agentId=agent_id, agentVersion=agent_version, knowledgeBaseId=knowledge_base_id
    )

    kb_response = bedrock_client.get_knowledge_base(knowledgeBaseId=knowledge_base_id)
    kb_status = kb_response["knowledgeBase"]["status"]

    data_sources = {}
    for key, ds in zip(
        ("static", "web"), ("data_source_s3", "data_source_web_crawler")
    ):
        if not user[ds]:
            continue
        data_sources[key] = {}
        ...
        if ds_status in ("DELETING", "DELETE_UNSUCCESSFUL"):
            data_sources[key]["synchronization"] = {"status": None, "stats": {}}
        ...
        "body": json.dumps(
            {
                "agent": {
                    ...
                    "isAgentReady": agent_status
                    in ("PREPARED", "NOT_PREPARED", "DELETING", "FAILED"),
                    "agentError": agent_status == "FAILED",
                },
                "knowledge": {
                    "knowledgeBaseStatus": kb_status,
                    "knowledgeBaseState": agent_kb_response["agentKnowledgeBase"][
                        "knowledgeBaseState"
                    ],
                    "isKnowledgeReady": kb_status
                    in ("ACTIVE", "FAILED", "DELETE_UNSUCCESSFUL"),
                    "knowledgeError": kb_status
                    in ("FAILED", "DELETING", "DELETE_UNSUCCESSFUL"),
...
def get_crawling_status(user_id):
    ...
    try:
        ...
        return {"status": "NOT CRAWLED", "stats": {}}
