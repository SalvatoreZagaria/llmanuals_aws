...
def lambda_handler(event, context):
    ...
    lambda_client = boto3.client(
        "lambda", region_name=os.getenv("AWS_REGION", "eu-west-2")
    )
        ...
        storageConfiguration={
            "rdsConfiguration": {
                "credentialsSecretArn": os.getenv(
                    "RDS_SECRET_ARN",
                    "arn:aws:secretsmanager:eu-west-2:851725385545:secret:LLManualsAurora-hfpK6Y",
                ),
                ...
                "resourceArn": os.getenv(
                    "RDS_DB_ARN",
                    "arn:aws:rds:eu-west-2:851725385545:cluster:llmanualsdev",
                ),
                "tableName": os.getenv(
                    "RDS_DB_TABLE_NAME", "bedrock_integration.bedrock_kb"
                ),
            },
            "type": "RDS",
        },
...
def create_s3_data_source(user_id, kb_id):
    return create_data_source(
        user_id,
        kb_id,
        os.getenv("S3_BUCKET_ARN", "arn:aws:s3:::llmanuals-knowledge-source"),
        f"S3_{user_id}",
    )
...
def create_web_data_source(user_id, kb_id):
    return create_data_source(
        user_id,
        kb_id,
        os.getenv("WEB_BUCKET_ARN", "arn:aws:s3:::llmanuals-knowledge-source-web"),
        f"WEB_{user_id}",
    )
...
def wait_for_agent_operation(agent_id: str, operation: str):
    while True:
        ...
        if agent_status == "FAILED":
            raise AgentPreparationFailed(
                f"Agent failed - AGENT ID: {agent_id}; OPERATION: {operation}"
            )
        return
