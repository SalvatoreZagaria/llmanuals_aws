...
def lambda_handler(event, context):
    ...
    cognito_client = boto3.client(
        "cognito-idp", region_name=os.getenv("AWS_REGION", "eu-west-2")
    )
    cognito_client.admin_disable_user(
        UserPoolId=os.getenv("USER_POOL_ID", "eu-west-2_JUVPtjO37"), Username=user_id
    )
    ...
    if error:
        logger.warning(
            f"Skipping DynamoDB cleanup due to previous errors - USER_ID: {user_id}"
        )
        return

    dynamodb_client.Table("crawler_task").delete_item(Key={"user_id": user_id})

    logger.info(
        f"Profile deactivated and user environment cleaned - USER_ID: {user_id}"
    )


def delete_data_source(kb_id: str, ds_id: str) -> bool:
    try:
        bedrock_client.delete_data_source(dataSourceId=ds_id, knowledgeBaseId=kb_id)
    except:
        logger.error(
            f"Deleting data source unsuccessful: KNOWLEDGE_BASE: {kb_id}, DATA_SOURCE: {ds_id}"
        )
...
def delete_knowledge_base(kb_id: str) -> bool:
    ...
    while True:
        ...
        try:
            ...
            if status == "DELETE_UNSUCCESSFUL":
                logger.error(
                    f"Deleting knowledge base unsuccessful: KNOWLEDGE_BASE: {kb_id}"
                )
                return False
        except bedrock_client.exceptions.ResourceNotFoundException:
            return True
