...
def lambda_handler(event, context):
    ...
    if "organization_name" in body:
        ...
        if not (
            ...
            and 3 <= len(organization_name) <= 100
        ):
            return {
                "statusCode": 400,
                "headers": {"Access-Control-Allow-Origin": "*"},
                "body": json.dumps(
                    {
                        "message": "Bad request - organization_name must be a string between "
    ...
    if "organization_description" in body:
        ...
        if not (
            ...
            and isinstance(organization_description, str)
            and len(organization_description) <= 1000
        ):
            return {
                "statusCode": 400,
                "headers": {"Access-Control-Allow-Origin": "*"},
                "body": json.dumps(
                    {
                        "message": "Bad request - organization_description must be a string with maximum "
    ...
    if not any((organization_name, organization_description)):
        return {
            ...
            "body": json.dumps(
                {
                    "message": "Bad request - one of [organization_name, organization_description] "
                    "required"
                }
            ),
        }
