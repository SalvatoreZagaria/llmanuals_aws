...
def lambda_handler(event, context):
    ...
    response = dynamodb_resource.batch_get_item(
        RequestItems={
            "single_scraper_task": {
                "Keys": [{"task_id": st, "user_id": user_id} for url, st in url_tasks]
            }
        }
    )
    single_tasks = {
        st["task_id"]: st
        for st in response.get("Responses", {}).get("single_scraper_task") or []
    }

    metadata = {}
    for url, st_id in url_tasks:
        st = single_tasks[st_id]
        metadata[url] = {"status": st["task_status"], "metadata": st["metadata"]}
    ...
    task_table.update_item(
        ...
        ExpressionAttributeValues={
            ...
            ":task_status": (
                "SUCCEEDED"
                if all([v["status"] == "SUCCEEDED" for v in metadata.values()])
                else "FAILED"
            ),
        },
    )
