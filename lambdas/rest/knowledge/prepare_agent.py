...
def lambda_handler(event, context):
    ...
    response = bedrock_client.create_agent_alias(
        agentAliasName=str(uuid.uuid4()), agentId=agent_id
    )
    ...
    if current_alias_id and current_version:
        ...
        bedrock_client.delete_agent_alias(
            agentAliasId=current_alias_id, agentId=agent_id
        )
        bedrock_client.delete_agent_version(
            agentId=agent_id, agentVersion=current_version, skipResourceInUseCheck=True
        )
        logger.info(f"Done in {time.time() - start}")

    return {
        "statusCode": 200,
        "body": json.dumps({"alias_id": alias_id, "agent_version": agent_version}),
    }
...
def get_alias(agent_id: str, alias_id: str):
    ...
    if response["agentAlias"]["routingConfiguration"]:
        agent_version = response["agentAlias"]["routingConfiguration"][0].get(
            "agentVersion"
        )
...
def wait_for_alias_operation(agent_id: str, alias_id: str):
    while True:
        ...
        if alias_status == "FAILED":
            raise AgentPreparationFailed(
                f"Agent alias failed - AGENT ID: {agent_id}; ALIAS ID: {alias_id}"
            )
